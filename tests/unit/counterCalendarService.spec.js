...
const setup = () => {
  ...
  const currentDate = ref(
    DateTime.fromObject({
      year: TEST_DATE_YEAR,
      month: TEST_DATE_MONTH,
      day: TEST_DATE_DAY,
    }),
  );

  const { currentDateString, calendarOptions } = useCounterCalendarService(
    counter,
    currentDate,
    () => {},
  );
...
describe("counterCalendarService.ts", () => {
  it("can format dates correctly", () => {
    ...
    expect(currentDateString.value).toEqual(
      `${TEST_DATE_YEAR}-${TEST_DATE_MONTH}-${TEST_DATE_DAY}`,
    );
  });
